...
-- 위 세개의 열은 별칭을 붙여서 조회
-- 조회할 때 부서번호를 기준으로 내림차순으로 정렬
-- 부서번호가 같다면 사원이름(First_name)을 기준으로 오름차순 정렬
desc employees;
...
--[문제5] 연봉이 12,000 이상 되는 직원들의 lastt_name 과 salary 조회
SELECT
    last_name,
    salary
FROM
...
--[문제7] 20번 혹은 50번 부서에서 근무하는 모든 사원의 LAST_NAME 및 department_id
--조회 후, last_name 의 오름차순, department_id 의 오름차순으로 정렬
SELECT
    last_name,
    department_id
FROM
...
--[문제9] 연봉이 2500,3500,7000이 아니며, 직업이 SA_REP, ST_CLERK인 사원의
--전체정보 조회
SELECT
    *
FROM
...
--[문제 11] '2004'년도에 고용된 모든 사람들의 last_name,hire_date 를 조회하여
--입사일 기준으로 오름차순 정렬
...
--LIKE 와 와일드 카드 사용    
--[문제 12]'2004'년도에 고용된 모든 사람들의 last_name,hire_date 를 조회하여
--입사일 기준으로 오름차순 정렬
SELECT
    last_name,
    hire_date
...
--[문제 13] last_name 에 u 가 포함되는 사원들의 사번 및 last_name 조회
SELECT
    employee_id,
    last_name
FROM
...
--[문제 14] last_name 의 네번째 글자가 a인 사원들의 사번 및 last_name 조회
SELECT
    employee_id,
    last_name
FROM
...
--[문제 15] last_name 에 a 혹은 e가 들어있는 사원들의 last_name 조회 후 
--last_name 오름차순 출력
SELECT
    employee_id,
    last_name
FROM
    employees
WHERE
    last_name LIKE '%a%' and '%e%'
ORDER BY
    last_name
asc;

--[문제 16] last_name 에 a 와 e가 들어있는 사원들의 last_name 조회 후 
--last_name 오름차순 출력
SELECT
    employee_id,
    last_name
...
--[문제 17] manager가 없는 사원들의 last_name, job_id 조회
SELECT
    last_name,
    job_id,
    manager_id
...
--[문제 18] st_clerk 인 job_id를 가진 사원이 없는 부서 id 조회
--단 부서번호가 null 인 값은 제외한다.
...
--[문제19] commision_pct 가 널이 아닌 사원들 중에서 commission = salary*commision_pct를
--구하여, employee_id, first_name, job_id 와 함꼐 출력
SELECT
    employee_id,
    first_name,
    job_id,
...
--[문제] first_name 이 Curtis 인 사람의 first_name, last_name, phone_number, job_id 조회
...
--[문제] 부서번호가 60,70,80,90 인 사원들의 employee_id, first_name, last_name, department_id
...
--[문제] job_id가 AD_PRES, PU_CLERK인 사원들의 employee_id, first_name, lsat_name, department_id
--job_id 조회하기, 단 사원명은 first_name과 last_name 을 연결하여 출력
SELECT
    employee_id,
...
--[실습] 부서 80의 각 사원에 대해 적용 가능한 세율 적용
-- 2000 미만 0%, 4000 미만 9%, 6000 미만 20%,
-- 8000 미만 30%, 10000 미만, 40%, 12000미만 42%,
-- 14000 미만 44%, 나머지 45%
SELECT
    last_name,
    salary,
...
--[문제] 부서별 직원의 수를 구하여, 부서번호의 오름차순으로 출력
SELECT
    department_id,
    COUNT(employee_id)
FROM
...
SELECT
    ...
    e.manager_id,
    e.hire_date    AS 내입사일,
    e1.employee_id AS mgr_id,
    e1.hire_date   AS mgr_hiredate
FROM
         employees e
...
-- [실습] 위치 ID 가 1700인 사원들의 last_name, salary 
...
-- [실습] 각 부서별 평균연봉(소수점 2자리까지), 사원 수 조회
